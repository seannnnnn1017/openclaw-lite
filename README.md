...
- `/clear`, `/clear-history`, `/clear-cache`: 清除 in-memory 對話歷史
- `/reload`: 重新載入 config / prompts / skills，並刷新 runtime clients
- `/status`: 顯示目前 model、history 長度、LLM URL、skill server URL

注意：

- `/clear-cache` 目前等價於清除對話歷史，不會刪掉檔案備份或排程資料
